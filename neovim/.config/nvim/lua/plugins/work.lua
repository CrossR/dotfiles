...
return {{
    ...
    ft = "markdown",
    config = function()
        vim.g.remarkjs_global = vim.fn.stdpath("config") .. "/assets/remarkjs_template.html"
        vim.g.remarkjs_local = vim.fn.stdpath("config") .. "/assets/local_remarkjs_template.html"
        ...
        vim.fn.Remark_Local = function()
            ...
            if vim.fn.filereadable(vim.fn.expand("%:p:h") .. "/remark_cache") == 0 then
                print("Cache missing, should be in " .. vim.fn.expand("%:p:h") .. "/remark_cache")
                return 1
            end
    ...
    keys =
        {{"<leader>rr", ":RemarkPreview<CR>"}, {"<leader>rl", ":call Remark_Local()<CR>"}}
}, {
    "lervag/vimtex",
    config = function()
        ...
        vim.g.vimtex_quickfix_ignore_filters = {".*file\\/after\\/translations-basic-dictionary-english\\.trsl.*",
    ...
    keys = {{"<F8>", ":VimtexCountWords<CR>"}, {"<F9>", ":VimtexCompile<CR>"}, {"<F10>", ":VimtexTocToggle<CR>"},
            {"<F11>", ":VimtexClean<CR>"}}
}, {
    "lervag/wiki.vim",
    ...
    config = function()
        ...
        vim.g.wiki_export = {
            args = {"-V geometry:margin=0.5in", "-H " .. git_default_dir .. "/wiki/pandoc/listings-setup.tex",
                    "--listings"},
